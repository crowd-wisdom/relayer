...
import type { IGetPrivateKeyData, IGetPublicKeyData, IGetZkeyFilePathsData } from "./types.js";
...
export class FileService {
  ...
  getZkeyFilePaths(name: string, mode?: EMode): IGetZkeyFilePathsData {
    ...
    if (!fs.existsSync(zkey) || (!fs.existsSync(wasm) && !fs.existsSync(witnessGenerator))) {
      this.logger.error(
        `Error: ${ErrorCodes.FILE_NOT_FOUND}, zkey: ${zkey} - ${fs.existsSync(zkey)}, wasm: ${wasm} - ${fs.existsSync(wasm)}, witnessGenerator: ${witnessGenerator} - ${fs.existsSync(witnessGenerator)}`,
      );
