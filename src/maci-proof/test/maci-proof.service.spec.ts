...
import * as maciContracts from '@maci-protocol/contracts'
dotenv.config();

let MaciProofServiceClass: any;

// mock parcial del SDK
const proofMock = {
  processProofs: [1],
  tallyProofs: [1],
  tallyData: {},
...
jest.mock('@maci-protocol/sdk', () => ({
  ...maciSdk,
  generateProofs: jest.fn(() => Promise.resolve(proofMock)),
  getPoll: jest.fn(() => Promise.resolve({ address: zeroAddress }))
}));

const ProofGeneratorMock = jest.fn();

jest.mock("@maci-protocol/contracts", () => ({
  ...maciContracts,
  Deployment: { getInstance: jest.fn() },
  ProofGenerator: ProofGeneratorMock,
}));

const { Deployment, ProofGenerator } = await import("@maci-protocol/contracts");
...
describe("ProofGeneratorService", () => {
  ...
  ProofGeneratorMock.mockReturnValue(defaultProofGenerator);
  ...
  const { MaciProofService } = await import("../maci-proof.service.js");
  MaciProofServiceClass = MaciProofService;
  });
...
test("should generate proofs properly for Qv", async () => {
  ...
  const service = new MaciProofServiceClass(
